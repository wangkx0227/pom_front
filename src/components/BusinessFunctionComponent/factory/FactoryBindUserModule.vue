      ...
      <el-table :data="factoryData" style="width: 100%" height="540">
        ...
        <el-table-column label="绑定用户" align="center" width="180">
          <template v-slot="{ row }">
            ...
            <el-cascader v-else :options="user_data_list" v-model="row.user_id_list"
...
export default {
  ...
  methods: {
    ...
    saveRow(row) {
      // 保存的数据 row
      this.loading = true;
      if (Array.isArray(row.user_id_list)) { // 如果是列表类型取最后一位id
        if (row.user_id_list.length === 2) {
          row.user_id = row.user_id_list.pop(); // 由于联机选择器是一个列表的值进行获取的，只需要获取最后一个元素即可(数据库也是根据前端菜单的id进行绑定的)
        } else {
          row.user_id = null;
        }
      }
