<template>
  <el-dialog title="当前角色权限菜单展示" :visible.sync="P_dialogShow" width="30%" :before-close="P_dialogClose">
    ...
    <span slot="footer" class="dialog-footer">
            ...
            <el-button type="primary" @click="saveData" :loading="S_loading"
                       v-if="method_list.includes('POST')">确 定</el-button>
        </span>
  </el-dialog>
</template>
...
<script>
export default {
  name: "PermissionsDialogAssembly",
  data() {
    return {
      P_loading: false,
      S_loading: false,
  ...
  methods: {
    ...
    handleCheckChange() {
      ...
      this.menu_id_list = []; // 存放前端菜单权限
      this.api_method_id_list = []; // 存放后端菜单接口
      ...
      for (let i = 0; i < this.menu_id_list.length; i++) {
        ...
        if (menu_type.length === 3) {
          let one_menu = {type: `${menu_type[0]}-${menu_type[1]}`, id: menu_type[1]}
