      ...
      <el-dialog title="公司添加" :visible.sync="dialogDisplayVar" width="35%" :before-close="handleClose">
        <el-form :model="addFirmForm" label-position="top" :rules="rules" ref="addFirmRef">
          ...
          <el-form-item label="名称缩写" prop="abbr">
            <el-input v-model="addFirmForm.abbr"></el-input>
          </el-form-item>
...
export default {
  ...
  data() {
    let descriptionLen = (rule, value, callback) => {
      if (this.addFirmForm.description.length >= 200) {
  ...
  methods: {
    ...
    dialogClose(formName) {
      ...
      this.getFirmData(); // 进行回调，重新载入一下数据
    },
    // ×关闭
    handleClose() {
      this.dialogDisplayVar = false;
    ...
    // 弹窗内创建按钮
    addFirmData(formName) {
      this.addLoading = true;
      if (!this.addFirmForm.firm) {
    ...
    getFirmData() {
      let get_url;
      if (this.search) {
        get_url = `foundation/firm/?page=${this.page}&search=${this.search}`;
      } else {
